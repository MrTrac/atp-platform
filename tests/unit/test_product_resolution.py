"""Unit tests for ATP M3-M4 resolution plus the v0.5-v1.0 contract chain."""
...
class TestProductResolution(unittest.TestCase):
    """Cover file-based resolution flow plus the v0.5-v1.0 contract chain."""
    ...
    def test_decision_to_closure_continuation_handoff_contract_is_explicit_and_narrow(self) -> None:
        normalized = normalize_request(load_request(FIXTURE_DIR / "sample_request_atp.yaml"))
        classification = classify_request(normalized)
        resolution = resolve_product(normalized, classification)
        resolution_contract = build_request_to_product_resolution_contract(
            run_id="run-v0-6-slice-b-1",
            normalized_request=normalized,
            classification=classification,
            resolution=resolution,
            manifest_id="task-manifest-req-1",
        ...
        contract = build_decision_to_closure_continuation_handoff_contract(
            run_id="run-v0-6-slice-b-1",
            normalized_request=normalized,
            resolution_contract=resolution_contract,
            handoff_intent_contract=handoff_contract,
        ...
        self.assertEqual(contract["request_id"], normalized["request_id"])
        self.assertEqual(contract["run_id"], "run-v0-6-slice-b-1")
        self.assertEqual(contract["handoff_scope"], "decision_to_closure_continuation_only")
        self.assertEqual(
            contract["post_execution_decision_ref"]["contract_id"],
            post_execution_decision_contract["contract_id"],
        )
        self.assertEqual(
            contract["closure_or_continuation_handoff"]["bounded_next_path"],
            "close",
        )
        self.assertEqual(
            contract["closure_or_continuation_handoff"]["next_record_type"],
            "closure_record",
        )
        self.assertEqual(
            contract["closure_or_continuation_handoff"]["review_escalation_mode"],
            "none",
        )
        self.assertEqual(
            contract["traceability"]["post_execution_decision_contract_id"],
            post_execution_decision_contract["contract_id"],
        )
        self.assertEqual(
            contract["traceability"]["product_execution_result_contract_id"],
    ...
    def test_closure_continuation_state_contract_is_explicit_and_narrow(self) -> None:
        normalized = normalize_request(load_request(FIXTURE_DIR / "sample_request_atp.yaml"))
        classification = classify_request(normalized)
        resolution = resolve_product(normalized, classification)
        resolution_contract = build_request_to_product_resolution_contract(
            run_id="run-v0-6-slice-c-1",
            normalized_request=normalized,
            classification=classification,
            resolution=resolution,
            manifest_id="task-manifest-req-1",
        ...
        contract = build_closure_continuation_state_contract(
            run_id="run-v0-6-slice-c-1",
            normalized_request=normalized,
            resolution_contract=resolution_contract,
            handoff_intent_contract=handoff_contract,
        ...
        self.assertEqual(contract["request_id"], normalized["request_id"])
        self.assertEqual(contract["run_id"], "run-v0-6-slice-c-1")
        self.assertEqual(contract["state_scope"], "closure_continuation_state_only")
        self.assertEqual(
            contract["decision_to_closure_continuation_handoff_ref"]["contract_id"],
            decision_to_handoff_contract["contract_id"],
        )
        self.assertEqual(
            contract["closure_or_continuation_state"]["bounded_path"],
            "close",
        )
        self.assertEqual(
            contract["closure_or_continuation_state"]["state_status"],
            "closed",
        )
        self.assertFalse(contract["closure_or_continuation_state"]["continuation_required"])
        self.assertFalse(contract["closure_or_continuation_state"]["review_escalation_active"])
        self.assertEqual(
            contract["traceability"]["decision_to_closure_continuation_handoff_contract_id"],
            decision_to_handoff_contract["contract_id"],
        )
        self.assertNotIn("selected_provider", contract)
        self.assertNotIn("selected_node", contract)
    ...
    def test_finalization_closure_record_contract_is_explicit_and_narrow(self) -> None:
        normalized = normalize_request(load_request(FIXTURE_DIR / "sample_request_atp.yaml"))
        classification = classify_request(normalized)
        resolution = resolve_product(normalized, classification)
        resolution_contract = build_request_to_product_resolution_contract(
            run_id="run-v0-7-slice-a-1",
            normalized_request=normalized,
            classification=classification,
            resolution=resolution,
            manifest_id="task-manifest-req-1",
        ...
        contract = build_finalization_closure_record_contract(
            run_id="run-v0-7-slice-a-1",
            normalized_request=normalized,
            execution_result_contract=execution_result_contract,
            post_execution_decision_contract=post_execution_decision_contract,
        ...
        self.assertEqual(contract["request_id"], normalized["request_id"])
        self.assertEqual(contract["run_id"], "run-v0-7-slice-a-1")
        self.assertEqual(contract["record_scope"], "finalization_closure_record_only")
        self.assertEqual(
            contract["closure_continuation_state_ref"]["contract_id"],
            closure_state_contract["contract_id"],
        )
        self.assertEqual(
            contract["finalization_or_closure_record"]["bounded_path"],
            "close",
        )
        self.assertEqual(
            contract["finalization_or_closure_record"]["record_status"],
            "closure_record_finalized",
        )
        self.assertEqual(
            contract["finalization_or_closure_record"]["final_status"],
            "completed",
        )
        self.assertFalse(contract["finalization_or_closure_record"]["continuation_required"])
        self.assertEqual(
            contract["traceability"]["closure_continuation_state_contract_id"],
            closure_state_contract["contract_id"],
        )
        self.assertEqual(
            contract["traceability"]["finalization_id"],
            "finalization-req-1",
        )
        self.assertNotIn("selected_provider", contract)
        self.assertNotIn("selected_node", contract)
    ...
    def test_review_approval_gate_contract_is_explicit_and_narrow(self) -> None:
        normalized = normalize_request(load_request(FIXTURE_DIR / "sample_request_atp.yaml"))
        classification = classify_request(normalized)
        resolution = resolve_product(normalized, classification)
        resolution_contract = build_request_to_product_resolution_contract(
            run_id="run-v1-0-slice-a-1",
            normalized_request=normalized,
            classification=classification,
            resolution=resolution,
            manifest_id="task-manifest-req-1",
        ...
        contract = build_review_approval_gate_contract(
            run_id="run-v1-0-slice-a-1",
            normalized_request=normalized,
            execution_result_contract=execution_result_contract,
            post_execution_decision_contract=post_execution_decision_contract,
        ...
        self.assertEqual(contract["request_id"], normalized["request_id"])
        self.assertEqual(contract["run_id"], "run-v1-0-slice-a-1")
        self.assertEqual(contract["gate_scope"], "review_approval_gate_only")
        self.assertEqual(
            contract["finalization_closure_record_ref"]["contract_id"],
            finalization_contract["contract_id"],
        )
        self.assertEqual(
            contract["review_or_approval_gate"]["gate_subject"],
            "finalization_closure_record",
        )
        self.assertEqual(contract["review_or_approval_gate"]["gate_decision"], "approved")
        self.assertEqual(contract["review_or_approval_gate"]["gate_status"], "passed")
        self.assertEqual(
            contract["traceability"]["finalization_closure_record_contract_id"],
            finalization_contract["contract_id"],
        )
    ...
    def test_gate_outcome_operational_followup_contract_is_explicit_and_narrow(self) -> None:
        normalized = normalize_request(load_request(FIXTURE_DIR / "sample_request_atp.yaml"))
        classification = classify_request(normalized)
        resolution = resolve_product(normalized, classification)
        resolution_contract = build_request_to_product_resolution_contract(
            run_id="run-v1-0-slice-b-1",
            normalized_request=normalized,
            classification=classification,
            resolution=resolution,
            manifest_id="task-manifest-req-1",
        ...
        contract = build_gate_outcome_operational_followup_contract(
            run_id="run-v1-0-slice-b-1",
            normalized_request=normalized,
            finalization_closure_record_contract=finalization_contract,
        ...
        self.assertEqual(contract["request_id"], normalized["request_id"])
        self.assertEqual(contract["run_id"], "run-v1-0-slice-b-1")
        self.assertEqual(contract["followup_scope"], "gate_outcome_operational_followup_only")
        self.assertEqual(
            contract["finalization_closure_record_ref"]["contract_id"],
            finalization_contract["contract_id"],
        )
        self.assertEqual(
            contract["review_approval_gate_ref"]["contract_id"],
            gate_contract["contract_id"],
        )
        self.assertEqual(
            contract["gate_outcome_or_operational_followup"]["bounded_followup"],
            "approved_operational_followup",
        )
        self.assertEqual(
            contract["gate_outcome_or_operational_followup"]["followup_status"],
            "operationally_ready",
        )
        self.assertEqual(
