...
- If you cannot access the required content, write a tight prompt for another AI to execute
  The prompt should be delivered in one copy/paste-ready block, tell the user to paste the result back here, make `verify` the next mandatory step, require that `verify` PASS, and only then allow a second review/follow-up prompt if needed
