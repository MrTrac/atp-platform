"""
ATP Adapter: cursor-subprocess

Bridges AIOS-OC FlowView's `cursor-agent` node to the Cursor `cursor-agent`
CLI subprocess on the local Mac.

Architecture (mirrors claude_code.py):
    AIOS-OC FlowView `cursor-agent` node
        → ATP bridge (POST /run, adapter="cursor")
            → CursorAdapter.execute()
                → subprocess: cursor-agent --model <model> -p <prompt>
                    → stdout collected back to AIOS-OC

Requires:
- `cursor-agent` CLI on PATH (install from Cursor docs — `cursor.com/docs/cli`).
- A locally signed-in Cursor account. The CLI uses the same auth as the IDE.
"""
...
class CursorAdapterConfig:
    """Configuration for the cursor adapter."""

    # Cursor's CLI delegates to its own backend routing when model="auto".
    # Explicit values are what Cursor accepts today (subject to change —
    # no-op mapping keeps future IDs working without an adapter change).
    MODEL_MAP = {
        "auto": "auto",
    ...
    def __init__(
        ...
        # by the cursor-agent subprocess flow, but captured so the dispatcher
...
class CursorAdapter:
    """ATP adapter for Cursor `cursor-agent` CLI subprocess execution."""

    ADAPTER_ID = "cursor"
    VERSION = "0.1.0"
    CLI_BIN = "cursor-agent"

    def __init__(self, config: CursorAdapterConfig) -> None:
        self.config = config
    ...
    async def validate(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if proc.returncode != 0:
                return False, f"{self.CLI_BIN} CLI not found in PATH — install from cursor.com/docs/cli"
    ...
    async def execute(
        ...
        """Execute Cursor agent CLI with the rendered prompt."""
